      ...
      <div className='w-[90%] lg:max-w-[60%] h-[600px] bg-white rounded-2xl flex justify-center items-center overflow-hidden border-2 border-[#1a1f23]'>
        ...
        <div className='w-full lg:w-[50%] h-full bg-white flex flex-col items-center p-2.5 gap-5'>
          ...
          <div className='relative flex items-center justify-start w-[90%] h-[50px] rounded-2xl mt-[30px] border-2 border-black'>
            <label
              className={`text-gray-700 absolute left-5 p-[5px] bg-white text-[15px] transition-all duration-200 pointer-events-none ${(inputClicked.name || name.length > 0) ? "top-[-15px] text-xs" : ""}`}
              htmlFor='name'
            >
              Enter Your Name
            </label>
          ...
          <button onClick={handleSignup} disabled={loading} className='w-[70%] px-5 py-2.5 bg-black text-white font-semibold h-12.5 cursor-pointer rounded-2xl mt-7.5'>
            {loading ? <ClipLoader size={30} color='white' /> : "Sign up"}
          </button>

          <p onClick={() => navigate("/signin")} className=' text-gray-800 cursor-pointer '>
