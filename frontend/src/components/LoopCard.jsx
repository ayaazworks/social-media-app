...
const LoopCard = ({ loop }) => {
    ...
    useEffect(() => {
        ...
        socket?.on("commentedLoop", (updatedData) => {
            ...
            dispatch(setpostData(updatedLoops))
        })
        return () => {
    ...
    return (
        <div className='w-full lg:w-[480px] h-[100vh] flex items-center justify-center border-l-2 border-r-2 border-gray-800 relative overflow-hidden'>
            ...
            <div className='absolute z-[100] top-[20px] right-[20px]' onClick={() => setIsMute(!isMute)}>
                {!isMute ? <FiVolume className='size-[25px] text-white font-semibold ' /> : <FiVolumeX className='size-[25px] text-white font-semibold' />}
            </div>
            ...
            <div className='w-full absolute h-[100px] bottom-[10px] p-[10px] flex flex-col gap-[10px]'>
                <div className='flex items-center gap-[5px]'>
                    <div className='w-[30px] h-[30px] md:w-[40px] md:h-[40px] border-2 border-black rounded-full cursor-pointer overflow-hidden' onClick={() => navigate(`${serverURL}/api/user/getprofile/${loop?.author}`)}>
                        <img src={loop.author?.profileImage || nodp}
                            alt=''
                            className='w-full object-cover'
                        />
                    </div>
                    <div className='w-[120px] truncate text-white font-semibold'>{loop.author?.userName}</div>
                ...
                <div className='absolute right-0 flex flex-col gap-[20px] text-white bottom-[150px] justify-center px-[10px]'>
                    ...
                    <div className='flex flex-col items-center cursor-pointer'>
                        <div><MdOutlineComment className='w-[25px] cursor-pointer h-[25px]' onClick={() => setShowComment(true)} /></div>
